...
class ConcatLeftAlign:
    ...
    def forward(self, batch):
        if "action" in batch:
            batch["action"] = self._concat(batch["action"], self.action_meta) # default [32, sum_action_dim]
            batch["action"], batch["action_dim_is_pad"] = self._pad(batch["action"], self.action_target_dim) # [32, D_action], [D_action]

        batch["state"] = self._concat(batch["state"], self.state_meta) # default [33, sum_state_dim]
        batch["state"], batch["state_dim_is_pad"] = self._pad(batch["state"], self.state_target_dim) # [33, D_state], [D_state]
    ...
    def _pad(x: torch.Tensor, dim: int):
        ...
        x_padded = pad(x, (0, pad_dim)) # [T, dim]
        mask = torch.zeros_like(x[0]).bool() # [source_dim]
        mask = pad(mask, (0, pad_dim), value=True) # [dim]
    ...
    def _concat(x: Dict[str, torch.Tensor], meta: Dict[str, Dict]):
        x = torch.cat([x[m["key"]] for m in meta], dim=-1) # [T, sum_dim]
        assert x.ndim == 2
        return x
