...
class BaseLerobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        for meta in self.action_meta:
            ...
            delta_timestamps[meta["lerobot_key"]] = [(t * global_sample_stride) / fps for t in range(-past_action_size, -past_action_size + action_size)]  # default [32]
    ...
    def _get_action(self, meta, lerobot_sample) -> torch.Tensor:
        ...
        action: torch.Tensor = lerobot_sample[lerobot_key] # default [32, raw_action_dim]
    ...
    def _get_state(self, meta, lerobot_sample) -> torch.Tensor:
        key, lerobot_key, raw_shape = meta["key"], meta["lerobot_key"], meta["raw_shape"]
        state: torch.Tensor = lerobot_sample[lerobot_key] # default [33, raw_state_dim]
    ...
    def _get_image(self, meta, lerobot_sample) -> torch.Tensor:
        key, lerobot_key, raw_shape = meta["key"], meta["lerobot_key"], meta["raw_shape"]
        image: torch.Tensor = lerobot_sample[lerobot_key] # default [33, C, H, W]
        if image.ndim == 3: # time dim will lost when obs_size is 1
            image = image.unsqueeze(0)        
        image = (image * 255).to(torch.uint8) # default [33, 3, H, W]
    ...
    def __getitem__(self, idx):
        ...
        sample = {
            ...
            "action": {}, # each key default [32, raw_action_dim]
            "state": {}, # each key default [33, raw_state_dim]
            "images": {}, # each key default [33, 3, H, W]
        }
        for meta in self.state_meta:
        ...
        sample["action_is_pad"] = lerobot_sample[f"{self.action_meta[0]['lerobot_key']}_is_pad"] # default [32]
        sample["state_is_pad"] = lerobot_sample[f"{self.state_meta[0]['lerobot_key']}_is_pad"] # default [33]
        sample["image_is_pad"] = lerobot_sample[f"{self.image_meta[0]['lerobot_key']}_is_pad"] # default [33]
    ...
    def get_dataset_stats(self, preprocessor: BaseProcessor):
        ...
        if not multi_thread:
            for episode_idx in tqdm(range(episodes_num), desc="Iterating dataset to get normalization"):
                ...
                for meta in self.state_meta:
                    ...
                    # (B, T, dim) B 是trainning sample 的数量，一个episode 有多少帧就可以拆成多少个trainning sample。
                    # 这里不同于普通的滑动窗口采样，我们采用了最后一帧重复的手段。
                    state_min[key].append(cur_state.amin(0)) # 对于给定时间t，给定维度k，覆盖所有B的统计量(T, dim)
        ...
        for meta in self.state_meta:
            ...
            # 遍历所有 episode 的统计量，对一个训练样本，每个时间步 t、每个维度 k，计算的最小值统计量
