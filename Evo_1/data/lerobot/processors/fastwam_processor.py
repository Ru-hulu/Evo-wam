...
class FastWAMProcessor(BaseProcessor):
    ...
    def preprocess(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        sample["image_is_pad"] = data["image_is_pad"] # default [33]

        # 2. image
        processed_images = []
        for meta in self.shape_meta["images"]:
            key, shape = meta["key"], meta["shape"]
            image = data["images"][key]  # default [33, 3, H, W]
        ...
        pixel_values = torch.stack(processed_images, dim=0) # default [num_cameras, 33, 3, H, W]
        
        if self.num_output_cameras > pixel_values.shape[0]:
            out = torch.zeros((self.num_output_cameras,) + pixel_values.shape[1:], device=pixel_values.device, dtype=pixel_values.dtype) # [num_output_cameras, 33, 3, H, W]
            out[0: pixel_values.shape[0]] = pixel_values
            sample["pixel_values"] = out # [num_output_cameras, 33, 3, H, W]
            ...
            sample["pixel_values"] = pixel_values[:self.num_output_cameras] # [num_output_cameras, 33, 3, H, W]
        else:
            sample["pixel_values"] = pixel_values # [num_cameras, 33, 3, H, W]
        ...
        if "action" in data and self.delta_action_dim_mask is not None:
            ...
            if bool(action_is_pad.any().item()):
                for key, dim_mask in self.delta_action_dim_mask.items():
                    cur_action = data["action"][key] # default [32, raw_action_dim]
                    cur_action_is_pad = action_is_pad.to(device=cur_action.device) # [32]
                    cur_dim_mask = dim_mask.to(device=cur_action.device) # [raw_action_dim]
                    pad_delta_mask = cur_action_is_pad.unsqueeze(1) & cur_dim_mask.unsqueeze(0) # [32, raw_action_dim]
        ...
        if "action" in data:
            sample["action"] = data["action"] # default [32, D_action]
            sample["action_is_pad"] = data["action_is_pad"] # default [32]
            sample["action_dim_is_pad"] = data["action_dim_is_pad"] # [D_action]
        ...
        sample["proprio"] = data["state"] # default [33, D_state]
        sample["proprio_is_pad"] = data["state_is_pad"] # default [33]
        sample["proprio_dim_is_pad"] = data["state_dim_is_pad"] # [D_state]
